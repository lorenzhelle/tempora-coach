...
- **SEC-002** — Strava-Access-Tokens laufen nach 6h ab. Der Refresh muss
  ...
  API-Aufruf fehlschlägt (siehe `SPECS.md` Spec 1, AC 4).
- **SEC-003** — Wird ein Refresh-Token ungültig (Nutzer hat Zugriff in Strava
  ...
  darf keine anderen Syncs blockieren (siehe `SPECS.md` Spec 1, AC 5).
...
- **DATA-002** — Strava-Aktivitäten werden über `stravaActivityId`
  ...
  darf nie zu doppelten `Activity`-Einträgen führen (siehe `SPECS.md`
  Spec 1, AC 3).
- **DATA-003** — Eine Chat-Anpassung ändert gezielt die betroffenen Felder
  eines Plans, nie den gesamten Plan neu generieren (siehe `SPECS.md`
  Spec 5, AC 1). Das verhindert, dass bereits bestätigte Planteile
...
- **ARCH-002** — v1 bleibt bewusst auf einen Nutzer beschränkt (kein
  ...
  `TICKETS.md`) und erfordert eine eigene ADR, bevor sie umgesetzt wird.
...
- Ein Coding-Agent darf ohne Rückfrage: Code gemäß `SPECS.md`/`TICKETS.md`
  implementieren, Tests schreiben/ausführen, Dokumentation gemäß diesem
  Kontext-Layer aktualisieren.
