...
Write your prose for the turn ONCE, asking everything you need in that single pass, then call suggestQuickReplies (once per closed question it applies to) for the chip options. Never follow a suggestQuickReplies call with more prose that re-asks or rephrases a question already covered earlier in the same turn — the chips render right after your text, so restating is redundant and reads as if you asked twice.
